...
app.post('/login', (req,res) => {
    ...
    // Submitting unknown' OR '1'='1 allows access for this query
    // let query = "SELECT title FROM user WHERE username = '"+sentUser+"' AND password = '"+sentPass+"'";

    // Submitting unknown" OR '1'="1 allows access for this query 
    let query = `SELECT title FROM user WHERE username = "${sentUser}" AND password = "${sentPass}"`; 

    console.log("Username " + sentUser)
    console.log("Password " + sentPass)
